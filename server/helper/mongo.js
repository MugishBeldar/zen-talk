...
class MongoConnect {
    ...
    static connectServer({ username, password, host, isGlobal, database, retryWrites }) {
        try {
            let mongoClient;
            let uri = 'mongodb+srv://';
    ...
    connectMaster() {
        try {
            ...
            MongoConnect.connectServer({ username, password, host, database, debug, isGlobal: false, retryWrites });
        } catch (e) {
    ...
    __getDBConnection(database) {
        ...
        const client = MongoConnect.connectServer({ username, password, host, database, replicaSet: replicaSet });
      ...
      __loadCollections(client) {
        fs.readdirSync(`${global.ROOT_PATH}/modules`)
          .filter((moduleDirectory) => {
            const foundFileObj = fs.statSync(`${global.ROOT_PATH}/modules/${moduleDirectory}`);
